...
import type { UserPreset } from './SettingsModal'
...
interface TextInputBarProps {
  ...
  presets: UserPreset[]
  onPresetsChange: (presets: UserPreset[]) => void
}

export const TextInputModal: React.FC<TextInputBarProps> = ({ isOpen, onClose, onSend, presets, onPresetsChange }) => {
  ...
  const startEdit = (idx: number) => {
    setEditingIdx(idx)
    setFormLabel(presets[idx].label)
    setFormText(presets[idx].text)
  }
  ...
  const deletePreset = (idx: number) => {
    onPresetsChange(presets.filter((_, i) => i !== idx))
  ...
  return (
    <div className="shrink-0 flex flex-col bg-bg-secondary border-t border-border-subtle">
      ...
      <div className="flex items-center gap-1.5 px-2 pt-1.5 overflow-x-auto no-scrollbar">
        <button
      ...
      {deleteConfirm !== null && (
        <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center px-4" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-bg-secondary border border-border-subtle rounded-xl shadow-2xl max-w-sm w-full p-4" onClick={e => e.stopPropagation()}>
            <p className="text-[13px] text-text-primary">Delete preset <span className="font-semibold text-accent-blue">{presets[deleteConfirm]?.label}</span>?</p>

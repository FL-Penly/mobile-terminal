...
export const BranchSelector: React.FC<BranchSelectorProps> = ({ currentBranch, onBranchChange }) => {
  ...
  const fetchBranches = useCallback(async () => {
    setIsLoading(true)
    try {
      const url = `http://${location.hostname}:${DIFF_SERVER_PORT}/api/git/branches`
  ...
  const handleCheckout = async (branch: string) => {
    setIsOpen(false)
    try {
      const url = `http://${location.hostname}:${DIFF_SERVER_PORT}/api/git/checkout?branch=${encodeURIComponent(branch)}`
